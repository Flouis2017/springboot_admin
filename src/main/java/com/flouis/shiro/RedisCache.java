...
/**
 * @description RedisCache<K , V> 实现 shiro Cache<K, V> 缓存接口，并重写 Cache<K, V> get、put、remove、clear、size、
 * keys、values等方法，这些方法都是 shiro 在对缓存的一些操作，就是当 shiro 操作缓存的时候都会调用相应的方法，我们只需重写这些相应的
 * 方法就可以把 shiro 的缓存信息存入到 redis 了。这就是一个优秀的开源框架所具备的扩展性，它提供了一个cacheManager缓存管理器，我们
 * 只需重写这个管理器即可。
 */
@Slf4j
public class RedisCache<K,V> implements Cache<K,V> {
    ...
    public RedisCache(RedisService redisService){
        this.cacheKey = MyConst.AUTHORIZATION_CACHE;
